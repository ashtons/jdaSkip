...
//#ifdef __7100PLUS__
//TAP_DESCRIPTION ("Skip manager 7100+/7160");
//#elif __7260__
//TAP_DESCRIPTION ("Skip manager 7260");
//#else
//TAP_DESCRIPTION ("Skip manager 2400/2100");
//#endif
...
//void TMS_Win_ShowMessageWin (char* title, char* lpMessage1, char* lpMessage2, dword dwDelay);
...
static dword key_handler( dword param1 )
{
	...
	switch (_phase)
	{
		...
		default:					DEBUG("Invalid _phase found: %d\n", _phase); break;
	}
...
dword TAP_EventHandler( word event, dword param1, dword param2 )
{
	...
	switch( event )
	{
		...
		case EVT_TMSCommander:	return TMSCommander_handler( param1 ); break;

		default:		DEBUG("Invalid event found: %d", event); 
			break;
	}
	return param1;
...
int TAP_Main (void)
{
	...
	//if (GetUptime() > (5*60*100))   //more than 5 minutes after system start?  1 tick = 10ms, 1 sec = 100 ticks
	//{
		//Show the user we're started and the settings
	//	sprintf(config, "%s: %d, %s: %d, %s: %d, %s: %d", 
	//		GetKeyName(key1), key1Len, GetKeyName(key2), key2Len, GetKeyName(key3), key3Len, GetKeyName(key4), key4Len);
	//	DEBUG("%s\n", config);
	//	TMS_Win_ShowMessageWin(PGM_FULL, "  jdaSkip TAP started  ", config, 3000);
	//}
