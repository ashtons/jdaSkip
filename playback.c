...
char      LogPuffer[256];
#define DEBUG(...)      if (TRC) {sprintf(LogPuffer, __VA_ARGS__); WriteLog(LogPuffer);}
...
static void UpdateGraphic()
{
	...
	if ((PRESSANDHOLD == PRESSANDHOLD_PERCENT || PRESSANDHOLD == PRESSANDHOLD_TIMEPERCENT) && 
		(_playInfo.playMode == PLAYMODE_Playing || _playInfo.playMode == PLAYMODE_AVI_DIVX || _playInfo.playMode == PLAYMODE_TS_MP4_MKV ))
	{
		dword target;
...
static bool IsSkippableMode(dword key) 
{
    ...
    if ((_subState != SUBSTATE_Normal) && 
		(_subState != SUBSTATE_PvrTimeSearch) && (_subState != SUBSTATE_PvrPlayingSearch) && (_subState != SUBSTATE_PvrRecSearch))
		return FALSE;
...
static dword CalculateSkip(bool haveTimeshifted)
{
	...
	if (secs < 0)
	{
		if (!haveTimeshifted && _playInfo.currentBlock == 0xFFFFFFFF) return 0;  //at start of buffer
	...
	if (MODE == SKIPMODE_BINARY)
	{
		...
		{
			if ((LastSkipDirection == SKIPDIR_FWD && !fwd) ||
				(LastSkipDirection == SKIPDIR_BACK && fwd))
			{
...
static bool SetTimeshift()
{
	...
	*/
	...
	if (!IsCurrentChannelRecording(&recInfo)) return FALSE;
...
dword handle_non_menu_keys( dword param1 )
{
	...
	if (!Repeating)
	{
		...
		if ((_subState == SUBSTATE_PvrTimeSearch || _subState == SUBSTATE_PvrPlayingSearch || _subState == SUBSTATE_PvrRecSearch) &&
			RESERVEKEYS && (tKey == GREENKEY || tKey == YELLOWKEY || tKey == BLUEKEY || tKey == RKEY_Prev || tKey == RKEY_Next))
		{
		...
		if ((_subState == SUBSTATE_PvrTimeSearch || _subState == SUBSTATE_PvrPlayingSearch || _subState == SUBSTATE_PvrRecSearch) &&
			(tKey == RKEY_Left || tKey == RKEY_Right || tKey == RKEY_Up || tKey == RKEY_Down))
		{
