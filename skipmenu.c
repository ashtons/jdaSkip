...
void ShowMenu()
{
	...
	if (bSkipStdEnabled)
	{
		//OSDMenuItemAdd("Skip Key 1", (char *)GetKeyName(key1), NULL, NULL, TRUE, TRUE, 0);
		OSDMenuItemAdd("Skip Key 1 Seconds", GetStringFromInt(key1Len), NULL, NULL, TRUE, TRUE, 0);
		//OSDMenuItemAdd("Skip Key 2", (char *)GetKeyName(key2), NULL, NULL, TRUE, TRUE, 0);
	}
	else
	{
		//OSDMenuItemAdd("Skip Key Back", (char *)GetKeyName(key1), NULL, NULL, TRUE, TRUE, 0);
		OSDMenuItemAdd("Skip Seconds", GetStringFromInt(key1Len), NULL, NULL, TRUE, TRUE, 0);
		//OSDMenuItemAdd("Skip Key Forward", (char *)GetKeyName(key2), NULL, NULL, TRUE, TRUE, 0);
	}
	OSDMenuItemAdd("Skip Key 2 Seconds", GetStringFromInt(key2Len), NULL, NULL, bSkipStdEnabled, TRUE, 0);
	//OSDMenuItemAdd("Skip Key 3", (char *)GetKeyName(key3), NULL, NULL, bSkipStdEnabled, TRUE, 0);
	OSDMenuItemAdd("Skip Key 3 Seconds", GetStringFromInt(key3Len), NULL, NULL, bSkipStdEnabled, TRUE, 0);
	//OSDMenuItemAdd("Skip Key 4", (char *)GetKeyName(key4), NULL, NULL, bSkipStdEnabled, TRUE, 0);
...
static void menuLeftRight_handler ( dword param1 )
{
	...
	switch (item)
	{
		case _optSkipMode:
			...
			{
				OSDMenuItemModifyValue(item, "Standard"); item += 2;
			...
			{
				OSDMenuItemModifyValue(item, "Binary"); item += 2;
...
dword menuactive_handler ( dword param1 )
{
	...
	if(InfoBoxOSDRgn)
	{
		...
		{
			case _optColourKeys:
				if (currentColourKey == 0) REDKEY = param1;
				if (currentColourKey == 1) GREENKEY = param1;
				if (currentColourKey == 2) YELLOWKEY = param1;
				if (currentColourKey == 3) BLUEKEY = param1;
				DEBUG("Current colour key = %s\n", ColourKeyNames[currentColourKey]);

				if (++currentColourKey > 3) return 0;
			...
			case _optSkipKeys:
				//set key counter to 1, type to Skip, and start prompting user for keys
				if (currentSkipKey == 0) key1 = param1;
				if (currentSkipKey == 1) key2 = param1;
				if (currentSkipKey == 2) key3 = param1;
				if (currentSkipKey == 3) key4 = param1;
				DEBUG("Current skip key = %s\n", SkipKeyNames[currentSkipKey]);

				if (++currentSkipKey > 3) return 0;
